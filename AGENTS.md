...
- For the current MVP, use file-backed camera capture, preserve image aspect ratio, and cap the long edge at `1024 px`. Do not reintroduce preview-bitmaps or square letterboxing unless a device log proves it is needed.
- For new Gemma 4 E2B Android app work, start from `docs/device-debugging/s23-plus-known-good-baseline.md` before changing model, backend, manifest, or preprocessing choices.
...
- Earlier 512x512 letterboxing was only a CPU/XNNPack diagnostic workaround. With S23+ GPU/OpenCL working, prefer aspect-ratio-preserving input up to `1024 px` long edge.
...
- 2026-05-11 S23+ Gallery comparison: Google AI Edge Gallery initialized `Gemma-4-E2B-it` Ask Image with `MainExecutorSettings: backend: GPU`, `EncoderBackend: GPU`, `AdapterBackend: CPU`, and OpenCL `LITERT_CL` delegates. At that time this app still fell from OpenCL to OpenGL and failed with `CreateSharedMemoryManager is not implemented`; PR #27 later fixed app-side OpenCL initialization through manifest native-library declarations.
...
- With `GPU only` and a prepared `1024 x 577` image, S23+ inference peaked around app `TOTAL PSS` `3.55 GB`, app `Graphics` `2.55 GB`, and system GPU memory `2.84 GB`, then returned to about app `TOTAL PSS` `241 MB`. Treat `1024 px` long edge as the current upper guardrail, not a casual default to raise.
