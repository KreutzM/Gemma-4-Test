...
- 2026-05-11 S23+ Gallery comparison: Google AI Edge Gallery did initialize `Gemma-4-E2B-it` Ask Image with `MainExecutorSettings: backend: GPU`, `EncoderBackend: GPU`, `AdapterBackend: CPU`, and OpenCL `LITERT_CL` delegates. This app still fell back from OpenCL to OpenGL and failed with `CreateSharedMemoryManager is not implemented`. Preserve this distinction in future GPU debugging.
